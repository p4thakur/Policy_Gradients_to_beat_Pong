...
import gym  #great to run  game################Universe is build on gym
...
# hyperparameters
H = 200 # number of hidden layer neurons..we will use 3 hidden layer
batch_size = 10 # every how many episodes to do a param update?
learning_rate = 1e-4 #for convergence (too low- slow to converge, too high,never converge)--0.0004
gamma = 0.99 # discount factor for reward (i.e later rewards are exponentially less important)
decay_rate = 0.99 # decay factor for RMSProp leaky sum of grad^2
resume = False # resume from previous checkpoint? should we load opur model now
...
grad_buffer = { k : np.zeros_like(v) for k,v in model.iteritems() }   # it help me to used back propagation way to store gradient
## rmsprop (gradient descent) memory used to update model
rmsprop_cache = { k : np.zeros_like(v) for k,v in model.iteritems() }   #store the value of formula
...
def prepro(I):  # i is game frame
  """ prepro 210x160x3 uint8 frame into 6400 (80x80) 1D float vector """
  I = I[35:195] # crop     # to crop my image from game, how 35, 195..i have to use it hit and trial
  I = I[::2,::2,0] # downsample by factor of 2  
  I[I == 144] = 0 # erase background (background type 1)
  I[I == 109] = 0 # erase background (background type 2)
  I[I != 0] = 1 # everything else (paddles, ball) just set to 1
  return I.astype(np.float).ravel() #flattens  # revel function udet to flateen in one d array
...
##short term reward means let say i get reward in current timestamp, and long term means i get rewards let say after
#30 minute so i may need to remeber all the tthings that have happens in last 30 min. In current game we don't need long term rewards
#since we are otimizing for short term reward, not long term reward (like legend of zelda)
def discount_rewards(r):  # we will weight reward differently as we are optimizing for short term(which mean i will remeber the short term information). r= conatins awards for every time stamps. we will wieght ech rewards differently,.
  """ take 1D float array of rewards and compute discounted reward """
  #initilize discount reward matrix as empty
  discounted_r = np.zeros_like(r)   # matrix of zeroes
  ...
  for t in reversed(xrange(0, r.size)):
    ...
    running_add = running_add * gamma + r[t]   # notes r is reverse , which mean r0 >r1>>r2..so we are xponentially decreasing our value  # this is discount reward formula for rieinforcement
    #earlier rewards given more value over time 
    #assign the calculated sum to our discounted reward matrix
    discounted_r[t] = running_add
  return discounted_r

# best thing about these code is i am learning backward and  forward propogation from scratch , no tenserflow
...
def policy_forward(x):
  ...
  h[h<0] = 0 # ReLU nonlinearity. one of the most used activation fucntion
...
observation = env.reset()  #take obervation from environment
prev_x = None # used in computing the difference frame(because thatt's hwo i can see motion between two frame)
...
while True:
  ...
  prev_x = cur_x
  #so x is our image difference, feed it in!(image different is my way of ditacting the motion)

  # forward the policy network and sample an action from the returned probability
  aprob, h = policy_forward(x)  # here i will get probailyt/gradient where in eed to move. Now i will have bunch of probaility so I will choose in next step which to choose. We choose probabilities stochasticily
  ...
  if done: # an episode finished
    ...
    #advatnage - quantity which describes how good the action is compared to the average of all the action.(discounted_epr is my advantage)
    epdlogp *= discounted_epr # modulate the gradient with advantage (PG magic happens right here.)(atle advantage and multiply by grad , to do backprop)
    ...
    if episode_number % batch_size == 0:
      for k,v in model.iteritems():
        ...
        rmsprop_cache[k] = decay_rate * rmsprop_cache[k] + (1 - decay_rate) * g**2  # RMSProp stochastic formula
        model[k] += learning_rate * g / (np.sqrt(rmsprop_cache[k]) + 1e-5)  # this is where value is updating..
